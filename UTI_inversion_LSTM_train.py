...
for speaker in speakers:
    ...
    # train-validation split, randomize data
    ult_training, ult_validation, mgc_lsp_training, mgc_lsp_validation = \
       train_test_split(ult, mgc, test_size=0.1, random_state=19)
    # ult_training = ult[0 : int(0.9*len(ult))]
    # ult_validation = ult[int(0.9*len(ult)) : ]
    # mgc_lsp_training = mgc[0 : int(0.9*len(mgc))]
    # mgc_lsp_validation = mgc[int(0.9*len(mgc)) : ]
    ...
    ### single training
    model = Sequential()
    model.add(LSTM(1000, input_shape=(n_sequence, n_mgc), kernel_initializer='normal', activation='relu', return_sequences=True))
    model.add(LSTM(750, kernel_initializer='normal', activation='relu'))
    ...
    model_name = 'models/UTI_inversion_LSTM_' + speaker + '_' + current_date
