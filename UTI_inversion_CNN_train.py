...
from keras.layers import Dense, Conv2D, UpSampling2D, Reshape
...
for speaker in speakers:
    ...
    model.add(Dense(16 * 24 * 32, kernel_initializer='normal', activation='relu'))
    model.add(Reshape((16, 24, 32)))
    model.add(UpSampling2D())
    model.add(Conv2D(32, kernel_size=5, padding='same', activation='relu'))
    model.add(UpSampling2D(size=2))
    model.add(Conv2D(1, kernel_size=5, padding='same', activation='linear'))
    ...
    model_name = 'models/UTI_inversion_CNN_' + speaker + '_' + current_date
